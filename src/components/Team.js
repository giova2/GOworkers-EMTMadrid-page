...
const Team = ({ tripData, stopsData }) => {
  ...
  const handleClickSeek = (from, homeUserAddress) => {
    ...
    planTravel(a, b).then((res) => {
      const trip =
        res.trip && res.trip.code == "00" ? res.trip : pruebaTrip.trip;
  ...
  const goWorkers = [
    {
      pic: `${window.location.href}images/profile.png`,
      name: "User1",
      position: "CEO",
      houseAddress: {
    ...
    {
      pic: `${window.location.href}images/profile2.png`,
      name: "User2",
      position: "Developer",
      houseAddress: {
  ...
  return (
    <Grid container spacing={2} style={{ marginBottom: "15px" }}>
      ...
      <Grid item xs={12} style={{ margin: "15px" }}>
        ...
        <Typography variant="h4">goWorkers</Typography>
      </Grid>
      <Grid item xs={12}>
        <Grid container={true} spacing={1}>
          {goWorkers.map(({ pic, name, position, houseAddress }) => {
            return (
              ...
              >
                <div
                  className={classes.goWorkerContainer}
                  onClick={() => {
                    setUserName(name);

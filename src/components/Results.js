...
const Results = ({ trip, selectStop, stopSelected}) => {
    ...
    const busSections = trip.data ? trip.data.sections.filter(elem => elem.type == 'Bus') : []
    return (
                        ...
                        <React.Fragment key={ segment.order }>
                            <Grid item xs={12} >
                                ...
                                <h3 className={ classes.titleResults }>
                                    <DirectionsBusIcon className={ classes.busIcon } /> Línea { segment.idLine }
                                </h3>
                            </Grid>
                            <Grid item md={6} xs={12} >
                                <Card className={`${classes.root} ${ (stopSelected ===  segment.source.properties.idStop) ? classes.selected : ''}`} onClick={ () => selectStop(segment.source.properties.idStop) }>
                                <CardContent>
                                    <Grid item xs={2}>
                            ...
                            <Grid item md={6} xs={12}>
                                <Card className={`${classes.root} ${ (stopSelected ===  segment.destination.properties.idStop) ? classes.selected : ''}`} onClick={ () => selectStop(segment.destination.properties.idStop) }>
                                <CardContent>
                                    <Grid item xs={2}>
                                        <Typography className={classes.title}>
